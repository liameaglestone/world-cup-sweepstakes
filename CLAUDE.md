...
- **Corner stat name confirmed**: The correct `s.name` for corners is `wonCorners` (label: "Corner Kicks"). The code checks `key === 'woncorners'` as the primary match.
...
- **Yellow cards & corners**: `data.boxscore.teams[].statistics[]` — find by `s.name` (lowercased). Yellow: `yellowcards`. Corner: `woncorners` (label: "Corner Kicks").
...
- Cache key is `wc26_matchCache_v3` — bump the version suffix when match processing logic changes, to bust stale cached `processed:true` entries.
